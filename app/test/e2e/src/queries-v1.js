const createQueryISOName = (iso) =>
    `SELECT iso, name_0 as name
        FROM gadm2_countries_simple
        WHERE iso in ${iso}`;

const createQueryID1 = (id, iso) =>
    `SELECT ST_AsGeoJSON(st_makevalid(the_geom)) AS geojson, (ST_Area(geography(the_geom))/10000) as area_ha
        FROM gadm28_adm1
        WHERE iso = UPPER('${iso}')
          AND id_1 = ${id}`;

const createQueryID1AndID2 = (id1, id2, iso) =>
    `SELECT ST_AsGeoJSON(st_makevalid(the_geom)) AS geojson, (ST_Area(geography(the_geom))/10000) as area_ha
...
const createQueryUSE = (cartodb_id, use_table) =>
    `SELECT ST_AsGeoJSON(st_makevalid(the_geom)) AS geojson, (ST_Area(geography(the_geom))/10000) as area_ha
        FROM ${use_table}
...
module.exports = { createQueryID1, createQueryISOName, createQueryGeometry, createQueryID1AndID2, createQueryUSE, createQueryWDPA };
